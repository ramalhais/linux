		...
		bool is_ram)
{
	...
	if (addr == DMA_MAPPING_ERROR) {
		pr_warn("dma_capable: DMA_MAPPING_ERROR\n");
		return false;
	}
	if (is_ram && !IS_ENABLED(CONFIG_ARCH_DMA_ADDR_T_64BIT) &&
	    min(addr, end) < phys_to_dma(dev, PFN_PHYS(min_low_pfn))) {
		pr_warn("dma_capable: min(addr, end) < phys_to_dma(dev, PFN_PHYS(min_low_pfn)): %x<%x. addr=%x min_low_pfn=%lx\n", min(addr, end), phys_to_dma(dev, PFN_PHYS(min_low_pfn)), addr, min_low_pfn);
		return false;
	}
	if (end > min_not_zero(*dev->dma_mask, dev->bus_dma_limit))
		pr_warn("dma_capable: end>bus_dma_limit\n");
